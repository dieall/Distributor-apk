...
<div class="flex flex-wrap items-center justify-between gap-2 mb-6">
    <div>
        ...
        <p class="text-secondary-light text-sm mb-0 mt-1">Buat dokumen pengiriman untuk permintaan yang sudah diproses sales</p>
    </div>
    <a href="{{ route('gudang.surat-jalan.index') }}" class="px-4 py-2 rounded-lg border border-neutral-300 dark:border-neutral-500 text-sm font-medium text-neutral-700 dark:text-neutral-200 hover:bg-neutral-100 flex items-center gap-2">
...
<div class="w-full space-y-6">
    {{-- Pilih permintaan (GET, terpisah dari simpan) --}}
    <div class="card shadow-none border border-neutral-200 dark:border-neutral-600 dark:bg-neutral-700 rounded-xl overflow-hidden w-full">
        ...
        <div class="p-6">
            <form method="GET" action="{{ route('gudang.surat-jalan.create') }}">
                ...
                <select name="permintaan_id" required onchange="this.form.submit()"
    ...
    @if($selected)
    <form action="{{ route('gudang.surat-jalan.store') }}" method="POST" id="form-sj">
        @csrf
        <input type="hidden" name="permintaan_id" value="{{ $selected->id }}">
        ...
        <div class="card shadow-none border border-neutral-200 dark:border-neutral-600 dark:bg-neutral-700 rounded-xl overflow-hidden w-full">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    ...
                    <div class="md:col-span-2 xl:col-span-3">
                        ...
                        <textarea name="alamat_tujuan" rows="2" placeholder="Alamat lengkap pengiriman"
                            class="w-full px-3.5 py-2.5 rounded-lg border border-neutral-300 dark:border-neutral-500 text-sm bg-white dark:bg-neutral-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 resize-none">{{ old('alamat_tujuan', $selected->pelanggan->address ?? '') }}</textarea>
                    </div>
                    <div class="md:col-span-2 xl:col-span-3">
                ...
                <button type="submit"
                    class="mt-6 w-full sm:w-auto px-8 py-2.5 rounded-lg bg-primary-600 hover:bg-primary-700 text-white text-sm font-medium inline-flex items-center justify-center gap-2 transition shadow-sm">
