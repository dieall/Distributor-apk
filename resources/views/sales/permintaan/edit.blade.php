...
@section('content')
@php
    $selectedItems = $permintaan->detail->keyBy('barang_id');
@endphp

<div class="flex flex-wrap items-center justify-between gap-2 mb-6">
    <div>
        <h6 class="font-semibold mb-0 dark:text-white">Edit Permintaan Barang</h6>
        <p class="text-secondary-light text-sm mb-0 mt-1">Perbarui barang yang Anda butuhkan</p>
    </div>
    <ul class="flex items-center gap-[6px] text-sm">
        <li class="font-medium"><a href="{{ route('sales.permintaan.index') }}" class="hover:text-primary-600">Permintaan</a></li>
...
<form action="{{ route('sales.permintaan.update', $permintaan) }}" method="POST">
@csrf
@method('PUT')
...
<div class="card shadow-none border border-neutral-200 dark:border-neutral-600 dark:bg-neutral-700 rounded-xl overflow-hidden mb-6">
    ...
    <div class="p-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            <div>
                <label class="block text-sm font-semibold text-neutral-700 dark:text-neutral-200 mb-1.5">Pelanggan</label>
                <select name="pelanggan_id" required class="w-full px-3.5 py-2.5 rounded-lg border border-neutral-300 dark:border-neutral-500 text-sm bg-white dark:bg-neutral-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500">
                    <option value="">Pilih Pelanggan</option>
                    @foreach($pelanggan as $p)
                        <option value="{{ $p->id }}" data-phone="{{ $p->phone }}" data-address="{{ $p->address }}" {{ old('pelanggan_id', $permintaan->pelanggan_id) == $p->id ? 'selected' : '' }}>{{ $p->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="block text-sm font-semibold text-neutral-700 dark:text-neutral-200 mb-1.5">Alamat Pengiriman</label>
                <input type="text" name="alamat" required value="{{ old('alamat', $permintaan->alamat) }}" placeholder="Masukkan alamat lengkap..."
            ...
            <div>
                ...
                <input type="text" name="no_po_customer" value="{{ old('no_po_customer', $permintaan->no_po_customer) }}" placeholder="Isi no po dari pelanggan (opsional)"
                    class="w-full px-3.5 py-2.5 rounded-lg border border-neutral-300 dark:border-neutral-500 text-sm bg-white dark:bg-neutral-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500">
            </div>
            <div>
                <label class="block text-sm font-semibold text-neutral-700 dark:text-neutral-200 mb-1.5">Nomor Handphone</label>
                <input type="text" name="no_hp" required value="{{ old('no_hp', $permintaan->no_hp) }}" placeholder="08xxxxxxxxxx"
                    class="w-full px-3.5 py-2.5 rounded-lg border border-neutral-300 dark:border-neutral-500 text-sm bg-white dark:bg-neutral-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500">
            </div>
            <div>
                <label class="block text-sm font-semibold text-neutral-700 dark:text-neutral-200 mb-1.5">Tanggal Dibutuhkan</label>
                <input type="date" name="tanggal_dibutuhkan" value="{{ old('tanggal_dibutuhkan', optional($permintaan->tanggal_dibutuhkan)->format('Y-m-d')) }}"
                    class="w-full px-3.5 py-2.5 rounded-lg border border-neutral-300 dark:border-neutral-500 text-sm bg-white dark:bg-neutral-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500">
            ...
            <div class="sm:col-span-2">
                ...
                <textarea name="catatan" rows="3" placeholder="Instruksi khusus, catatan pengiriman, dll..."
                    class="w-full px-3.5 py-2.5 rounded-lg border border-neutral-300 dark:border-neutral-500 text-sm bg-white dark:bg-neutral-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 resize-none">{{ old('catatan', $permintaan->catatan) }}</textarea>
            </div>
        </div>
    </div>
</div>
        ...
        <table class="w-full text-sm table-fixed min-w-[720px]" id="tabel-katalog">
            ...
            <tbody class="divide-y divide-neutral-100 dark:divide-neutral-600">
                ...
                @php
                    ...
                    $detail = $selectedItems->get($b->id);
                    $isChecked = old('barang_id') ? in_array($b->id, old('barang_id', [])) : (bool) $detail;
                    $qty = old('jumlah_diminta.' . $b->id, $detail?->jumlah_diminta ?? 1);
                @endphp
                <tr class="hover:bg-neutral-50/80 dark:hover:bg-neutral-600/40 transition-colors align-middle">
                    <td class="px-3 py-3 text-center">
                        <input type="checkbox" name="barang_id[]" value="{{ $b->id }}" class="check-item w-4 h-4 rounded border-neutral-300 text-primary-600 cursor-pointer" {{ $isChecked ? 'checked' : '' }}>
                    </td>
                    <td class="px-3 py-3">
                    ...
                    <td class="px-3 py-3">
                        <input type="number" name="jumlah_diminta[{{ $b->id }}]" value="{{ $qty }}" min="0.01" step="0.01"
                            class="input-jumlah w-full max-w-[7rem] mx-auto block px-2.5 py-2 rounded-lg border border-neutral-300 dark:border-neutral-500 text-sm text-center bg-white dark:bg-neutral-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500" {{ $isChecked ? '' : 'disabled' }}>
                    </td>
                </tr>
                @empty
                <tr>
