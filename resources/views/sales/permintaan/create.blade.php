...
<form action="{{ route('sales.permintaan.store') }}" method="POST">
@csrf
    ...
    <div class="p-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            <div>
                ...
                <select name="pelanggan_id" required class="w-full px-3.5 py-2.5 rounded-lg border border-neutral-300 dark:border-neutral-500 text-sm bg-white dark:bg-neutral-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500">
                    ...
                    @foreach($pelanggan as $p)
                        <option value="{{ $p->id }}" data-phone="{{ $p->phone }}" data-address="{{ $p->address }}">{{ $p->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
            ...
            <div>
                ...
                <input type="text" name="no_po_customer" placeholder="Isi no po dari pelanggan (opsional)"
                    class="w-full px-3.5 py-2.5 rounded-lg border border-neutral-300 dark:border-neutral-500 text-sm bg-white dark:bg-neutral-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500">
            </div>
            <div>
