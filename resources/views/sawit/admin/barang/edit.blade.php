...
@section('title', 'Edit Barang Sawit')

@section('content')
<div class="flex flex-wrap items-center justify-between gap-2 mb-6">
    <div>
        <h6 class="font-semibold mb-0 dark:text-white">Edit Barang Sawit</h6>
        <p class="text-secondary-light text-sm mb-0 mt-1">Perbarui data barang sawit</p>
    </div>
...
<div class="card shadow-none border border-neutral-200 dark:border-neutral-600 dark:bg-neutral-700 rounded-xl overflow-hidden">
    <div class="px-6 py-4 border-b border-neutral-200 dark:border-neutral-600 bg-neutral-50 dark:bg-neutral-800 flex items-center gap-3">
        ...
        <div>
            <h6 class="font-semibold mb-0 dark:text-white text-sm">Form Edit Barang</h6>
            <p class="text-secondary-light text-xs mb-0">{{ $barang->id_sawit }} - {{ $barang->nama_sawit }}</p>
        </div>
    </div>

    <div class="p-6">
        <form action="{{ route('sawit.admin.barang.update', $barang) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="id_sawit" value="{{ old('id_sawit', $barang->id_sawit) }}" placeholder="Contoh: SWT-0001" maxlength="50" required
                ...
                <div>
                    ...
                    <input type="text" name="nama_sawit" value="{{ old('nama_sawit', $barang->nama_sawit) }}" placeholder="Contoh: TBS (Tandan Buah Segar)" required
                ...
                <div class="md:col-span-2">
                    <label class="block text-sm font-semibold text-neutral-700 dark:text-neutral-200 mb-1.5">Total Kg Saat Ini</label>
                    <div class="px-3.5 py-2.5 rounded-lg border border-neutral-300 dark:border-neutral-500 bg-neutral-50 dark:bg-neutral-800">
                        <span class="text-lg font-bold {{ $barang->total_kg > 0 ? 'text-success-600' : 'text-danger-600' }}">
                            {{ number_format($barang->total_kg, 2) }} Kg
                        </span>
                        <p class="text-xs text-secondary-light mt-1">Stok akan terupdate otomatis dari transaksi pembelian dan penjualan</p>
                    </div>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="keterangan" rows="3" placeholder="Keterangan tambahan tentang barang sawit..."
                        class="w-full px-3.5 py-2.5 rounded-lg border border-neutral-300 dark:border-neutral-500 text-sm bg-white dark:bg-neutral-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-success-500 resize-none">{{ old('keterangan', $barang->keterangan) }}</textarea>
                    @error('keterangan')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror>
                </div>

                <div class="md:col-span-2">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $barang->is_active) ? 'checked' : '' }} class="w-4 h-4 rounded border-neutral-300 text-success-600 focus:ring-success-500">
            ...
            <div class="flex items-center justify-end gap-3 mt-6 pt-5 border-t border-neutral-200 dark:border-neutral-600">
                ...
                <button type="submit"
                    ...
                    <iconify-icon icon="ri:save-line"></iconify-icon> Update Barang
                </button>
            </div>
        </form>
    </div>
